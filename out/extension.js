...
class GrepResult extends vscode.TreeItem {
    ...
    sortGrepResult(a) {
        return this.path.localeCompare(a.path); // Sort alphabetically by label
    }
}
const sortTreeItems = (a, b) => {
    return a.sortGrepResult(b);
};
class GrepResultsTree {
    constructor() {
    ...
    getChildren(element) {
        if (!element) {
            // sort root items
            return this.items.sort(sortTreeItems);
        }
        else {
            // Sort child items
            return [];
...
function grepExec(query) {
    ...
    for (let cmdIndex = 0; cmdIndex < GREP_COMMANDS.length; ++cmdIndex) {
        try {
            ...
            const grepOutput = (0, child_process_1.execSync)(`${grepCmd} ${query} *`, { cwd: cwd, maxBuffer: 10 * 1024 * 1024 });
